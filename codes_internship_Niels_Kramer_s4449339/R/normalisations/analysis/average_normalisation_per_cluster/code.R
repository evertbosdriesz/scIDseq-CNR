...
for (cluster_no in c(1:length(unique(TMM_normalised$cluster)))) {
  ...
  TMM_average_normalised <- rbind(TMM_average_normalised, cluster_df)
}  

TMM_average_normalised_long <- TMM_average_normalised %>% 
  gather(key = 'antibody', value = 'intensity', -sample_id, -plate_number, -treatment, -cluster)

# save new data frame in the normalised data_folder
if(save_table == "yes") {
  write.csv(TMM_average_normalised, file = paste(data_dir, "/", output_file, sep = ""))
