...
```{r}
ggplot(
    filter(dat, ab_name == "RSK1_P_S380"),
    aes(y = zscore, fill = treatment)
  ) +
    geom_boxplot() +
    #ggbeeswarm::geom_beeswarm() +
    #expand_limits(y = 0) +
    my_theme +
    scale_fill_manual(values = col_lst) 
    labs(title = "RSK1_P_S380", x = "Cell state cluster", y = "z-score normalized ab-expression")


ggplot(
    filter(dat, ab_name == "RSK1_P_T359"),
    aes(y = zscore, fill = treatment)
  ) +
    geom_boxplot() +
    #ggbeeswarm::geom_beeswarm() +
...
In total `r model_anova |> filter(term == "treatment:cluster" & fdr < 0.05) |> nrow()` antibodies have a significant interaction ($p<0.05$) according to a one-way ANOVA analysis


```{r write supproting table lm model}
writexl::write_xlsx(
...
Here we just check that using only TMM normalized data (i.e. without subsequent z-score normatization) gives identical p-values for the two-way anova and the regression model.
