...
#run Leiden Alg. 5 times (for cell state marker assignment check DataOverview ...)
for (i in c(1:5)) {
  ## CODE
  ...
  if (save_figure == "yes") {
    pdf(file=paste(figure_dir, "/information_clusters_after_", no_repeats, "_repeats_of_leiden_filtered_", counter, ".pdf", sep = ""),
        width=20, height=10)
  }
  
  plot_a <- ggplot(data = tibble(no_of_clusters), aes(x = no_of_clusters)) +
  ...
  if (save_table == "yes") {
    write.csv(TMM_normalised_clusters, paste(data_dir, "/clusters_after_", no_repeats, "_repeats_of_leiden_filtered_", counter, ".csv", sep = "")) 
  }
  ...
  heatmap_a <- Heatmap(TMM_clusters, 
  ...
  heatmap_b <- Heatmap(cor_clusters, 
  ...
  if (save_table == "yes") {
    write.csv(cor_clusters, paste(data_dir, "/cooccurences_after_", no_repeats, "_repeats_of_leiden_filtered_", counter, ".csv", sep = "")) 
  }
  ...
  print("Done running Leiden iterations")
  
  if (save_table == "yes") {
    write.csv(TMM_final_cluster, paste(data_dir, "/TMM_normalised_weighed_clusters_after_", no_repeats, "_repeats_of_leiden_filtered_", counter, ".csv", sep = "")) 
  }
  ...
  plot_b <- ggplot(data = df_check, aes(x = cluster, y = count, fill = treatment)) +
  ...
  plot_c <- ggplot(TMM_final_cluster, aes(x = cluster, fill = treatment)) +
    geom_bar() +
    scale_fill_manual(values = palette_plots) +
    ggtitle("Amount of treatment per cluster") +
    labs(subtitle="Clustered on cell state marker antibodies") + 
  ...
  #create scatter plot of umap
  umap_treatment <- ggplot(TMM_final_cluster) + 
  ...
  print(umap_treatment)
  
  #create scatter plot of umap
  umap_plate_number <- ggplot(TMM_final_cluster) + 
  ...
  # add 1 to counter for next round
  counter = counter + 1
...
if (save_table == "yes") {
  print("place tables with the cluster assignments in the folder /clean_data/next_step")
}
